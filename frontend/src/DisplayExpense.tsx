...
const DisplayExpense = () => {
    ...
    useEffect(()=>{
            axios.get("http://localhost:3003/get").then((result)=>setExpense(result.data)).catch((err)=>console.log(err))
    },[])
    useEffect(()=>{
            axios.get("http://localhost:3003/totals").then((result)=>setCat(result.data)).catch((err)=>console.log(err))
    },[])
  return (
    <div className="body">
        ...
        <CreateExpense />
        <div className="row">
        {expense.length===0?<div>No expenses</div>:expense.map((exp)=>(
