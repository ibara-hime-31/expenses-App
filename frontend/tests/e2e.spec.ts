...
test("can add an expense and see it in the list", async ({ page }) => {
  ...
  await page.goto("/")
  await expect(page.getByRole("heading", { name: "Expenses Report" })).toBeVisible()

  await page.getByPlaceholder("Enter the title of the expense").fill(uniqueTitle)
  await page.getByPlaceholder("Enter the amount of the expense").fill(cost)
  await page.getByPlaceholder("Enter the category of the expense").fill(category)
